...
**API配置错误修复**
- 修复了FAL API密钥配置问题：将无效的占位符密钥改为演示模式
- 修复了豆包API模型名称错误：从 `doubao-seedream-3-0-t2i-250415` 更新为 `doubao-pro-128k-240515`
- 修复了R2存储配置问题：暂时禁用R2存储，避免上传失败错误
- 添加了API密钥验证逻辑，当密钥无效时自动切换到演示模式

**服务稳定性提升**
- FluxKontextService: 添加了FAL密钥有效性检查，无效时返回演示图片URL
- DoubaoService: 添加了API密钥验证，避免无效请求导致的错误
- 所有图像生成服务现在在API不可用时提供优雅降级
- 移除了会导致应用崩溃的硬错误，改为警告信息
